...
## 9. 4차 — 실기기 실측 (2026-09-19 · #193)

**근거는 사용자 보고다.** 사용자(P1)와 팀원(P2)이 탄천에서 안드로이드 폰 2대 · 계정 4개(카카오 A · B ·
구글 A · B)로 #193 의 「진행 계획」 1~5 를 돌렸고, 그 결과를 이 세션에 옮겼다. 원문은 #193 댓글
(2026-09-19)이다. **지휘관이 직접 본 것이 아니다.**

- 대상: integration Preview `tanchunrun-git-develop-…` (`develop` = `dbd7894`. 코드는 production `dd318d1` 과 같다)
- 실제 카카오 · 구글 로그인 · 실제 GPS · 실제 기기
- **iOS 는 범위 밖이다**(#78 D15 · 2026-09-18). 팀에 iPhone 이 없다

### 9-1. 결과

| 항목 | 결과 | 판정 |
| --- | --- | --- |
| F3(#146) 닉네임 중복 | 입력 중에는 잡지 않고 **「시작하기」 제출 시** 「이미 사용중인 닉네임입니다」. 오류 화면 없음 | ✅ 통과 — 실시간 검사는 계약이 아니다 |
| 위치 권한 거부 → 사이트 설정 허용 | **홈이 풀리지 않았다.** 다만 **폰 OS 의 GPS 토글은 정상 복구** | ❌ → **#200** |
| #190 멈춤 안내 | 권한 팝업을 둔 채 기다렸는데 안내 한 줄 · 「다시 확인」이 **안 떴다**. **대기 시간 미기록**(임계 15초) | ⚠️ 확정 전 → **#201** |
| 경계 판정(+2) | 일반 구간과 탄천 인정 구간이 분리 | ✅ 통과 |
| S7 두 계정 · 랭킹 | 1위(0.2)를 0.25 가 제치고, 0.2 가 누적 합산으로 1위 탈환까지 확인 | ✅ 통과 |
| 화면 꺼짐 2분 | 시간만 흐르고 경로는 늘지 않음 | ✅ **D1 대로** — 사용자가 알 길이 없다 → **#202** |
| 종료 · 기록 탭 · (+3) | 거의 동시 종료 · 결과 = 기록 탭 순위. 「연결이 끊겨…」 **재현 안 됨** | ✅ 통과. 8-8 의 관찰은 1회성으로 남긴다 |
| H3 다기기 | 동시 로그인 성공 · 양쪽 시작 시도 시 「이미 진행 중」 · 기록은 한 세션에만 | ⚠️ **부분** — P7 통과, **D14 인수 흐름 미검증** |
| P11 계정 분리 | 카카오 · 구글이 별개 계정 | ✅ 통과 |
| S8 탈퇴 · 재가입 | 사용자가 **어제까지 DB 를 함께 보며** 탈퇴 · 재가입 정상을 확인했다고 보고 | ✅ 통과(사용자 보고 · DB 확인 포함) |

### 9-2. 판정 갱신 — 8-4 를 이것으로 대체한다

바뀐 줄만 적는다. 나머지는 8-4 · 2절 그대로다.

| 항목 | 3차 | 4차 | 근거 |
| --- | --- | --- | --- |
| **Auth / User** | 미검증 | **통과** | 실제 카카오 · 구글 로그인 · 로그인 취소 · 다기기 · 탈퇴 후 재가입(9-1) |
| **Signup / Consent / Nickname** | 미검증(F3 재실측 없음) | **통과** | F3 중복 거절을 실기기에서 확인(9-1) |
| **Location / GPS** | 미검증 | **부분 통과** | 실기기 GPS · 화면 꺼짐 구간은 확인. **권한 복구(#200) · 멈춤 안내(#201) 미해결** |
| **Running Measurement** | 통과(로컬) | **통과(실기기)** | 실제 GPS 경로 · 경계 분리(9-1) |
| **Finish** | 통과(로컬) | **통과(실기기 + integration)** | 9-1 종료 · 8-8 실패 B |
| **Ranking** | 통과(로컬) | **통과(실기기)** | S7 순위 역전 · 누적 합산(9-1) |
| **Home · Result · Records · Personal Best** | 통과(로컬) | **통과(실기기)** | 결과 = 기록 탭 일치(9-1) |
| **Withdrawal** | 미검증(integration) | **통과(사용자 보고)** | 9-1. 지휘관이 직접 본 것은 아니다 |
| **Map(NAVER)** | 미검증 | **통과** | 실기기에서 지도 · Zone · 내 위치 표시(9-1 3번 러닝) |
| **Failure** | 부분 통과 | **부분 통과(유지)** | 실패 B 는 8-8 로 채워졌다. **`src/app` 의 `catch` 전수 확인은 여전히 안 했다** |
| **E2E** | 통과(로컬) | **통과(실기기)** | 9-1 전 구간 |

**전체 판정 — 아직 Gate 통과로 올리지 않는다.** 남은 것은 아래 셋이다.

1. **D14 인수 흐름 미검증** — 읽기 전용 러닝 화면 · 「이 기기에서 이어서 측정」 · 「여기서 종료」 · 인수 후 원래 기기 안내(#148).
   실측에서 폰 B 가 이 화면에 닿지 못했다. 재시도할 때는 **폰 B 에서 앱을 완전히 닫았다 다시 열어** `/running` 으로 가는지부터 본다
2. **미해결 결함 2건** — #200(권한 복구) · #201(멈춤 안내 · 결함 여부 확정 먼저)
3. **`src/app` 의 `catch` 전수 확인**

`#202`(화면 꺼짐 안내)는 **D1 대로 동작한 것에 대한 UX 보완이라 Gate 판정을 막지 않는다.**

### 9-3. 이번 회차에서 하지 않은 것

- iOS · Safari — 범위 밖(#78 D15)
- 실패 A(API 차단) 재실측 — 3차 · 8-2 의 로컬 결과를 그대로 둔다
- `#201` 의 30초 대기 — 현장에서 시간을 재지 않았다

### 9-4. `src/app` 의 `catch` 전수 확인 (2026-09-19 · 지휘관)

2절 Failure 줄이 「화면(`src/app`) 쪽 `catch` 는 전수 확인하지 않았다」로 남아 있던 것을 확인했다.
대상은 `src/app` 의 `catch` · `.catch(` **33곳**이다.

**가짜 성공은 없다.** 실패했는데 성공했다고 사용자에게 말하는 곳은 없었다. 대부분 이유가 주석에 있고
타당하다 — JSON 파싱 실패 → `400`, `sessionStorage` 차단, Wake Lock 미지원 · 거부, 권한 조회 실패,
응답 본문 파싱 실패(`response.json().catch(() => null)`), 서버 오류 화면을 그리는 `page.tsx` 들.

**관찰 2건** — 둘 다 Issue 로 만들지 않았다. 위험이 낮고, 지금 동작을 바꿀 근거가 없다.

| 곳 | 내용 | 판단 |
| --- | --- | --- |
| `useRunTracker.ts:668-677` | 복구 경로의 `readBufferedPoints → []` · `maxKnownRawSeq → 0` 이 **주석 없이** 조용히 떨어진다. 바로 아래 `readTerminalRawSeqs`(#164)에는 왜 빈 값이어도 되는지 근거가 적혀 있다. 둘 다 실패하면 다음 번호를 `서버 max + 1` 로 잡아 아직 못 보낸 점과 번호가 겹칠 수 있다 | **낮음** — IndexedDB 읽기가 실패해야 하고, 겹쳐도 서버가 `point_conflict` 로 격리하고 화면은 #151 안내를 띄운다. 다음에 이 파일을 열 때 근거 주석을 보태면 충분하다 |
| `ResultRecoveryGate.tsx:270 · 372` · `WithdrawButton.tsx:67` | 로컬 정리 실패를 삼킨다(`.catch(() => undefined)`) | **의도된 판단.** 정리 실패로 사용자를 화면에 가두지 않는다는 이유가 주석에 있다. 다만 **탈퇴 뒤 기기에 데이터가 남아도 사용자도 우리도 모른다** — 남는 사실로 적어 둔다 |

### 9-5. D14 인수 흐름 — 확인했다 (2026-09-19 · 사용자 보고)

9-1 에서 미검증으로 남았던 항목이다. **폰 1대 + 데스크톱 Chrome** 으로 같은 계정에 로그인해 다시 했다.

- 폰에서 러닝 시작 → 데스크톱에서 `/running` 진입 → **읽기 전용 화면과 「이 기기에서 이어서 측정」 ·
  「여기서 종료」 두 버튼**이 보였다
- 데스크톱에서 「여기서 종료」 → 확인 → 결과 화면
- 폰에서 「이 기기에서는 더 이상 측정하지 않아요」 안내(#148)

**사용자 보고다** — 지휘관이 직접 본 것은 아니다. 9-1 의 다기기 항목(P7 만 통과)을 이것으로 보완한다.

### 9-6. 4차 마무리 (2026-09-19)

9-2 가 남겨 둔 세 가지가 모두 정리됐다.

| 9-2 의 남은 항목 | 결과 |
| --- | --- |
| D14 인수 흐름 미검증 | **확인됨**(9-5) |
| #200 · #201 | **둘 다 재현되지 않아 닫았다.** 결함으로 확정하지 못했고, 다시 열 조건을 각 Issue 댓글에 적어 뒀다. 코드는 바꾸지 않았다 |
| `src/app` 의 `catch` 전수 확인 | **했다**(9-4). 가짜 성공 없음 · 관찰 2건 |

그 밖에 #193 에서 나온 #202(화면 꺼짐 안내)는 구현 · 실기기 확인 후 production 에 반영했다(PR #204 · 승격 #205).

**남는 것 — 판정을 바꾸지 않는 것들**

- **iOS · Safari** — 범위 밖(#78 D15)
- **320px 폭에서 #202 안내 줄바꿈** — 눈으로 재지 않았다
- 위 9-4 의 관찰 2건
- 여기 적힌 실기기 결과는 **전부 사용자 보고**다. 지휘관이 직접 본 것은 로컬 하네스 · integration 실측(8-8) 뿐이다

**#89 는 2026-09-19 에 닫았다.** 「Gate 통과」를 선언하며 닫은 것이 아니라, 팀이 MVP 검증을 여기까지 하기로 하고
남은 것은 개별 Issue 로 따라가기로 한 것이다. 그 뒤 이 문서가 채운 것이 9-4 · 9-5 · 9-6 이다.
