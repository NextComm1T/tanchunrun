 ...
 * 깨진다(이슈 「확인된 사실」). `sessionStorage` 는 탭 하나에 묶여 있어 **이미 열려 있던
 * 다른 탭**의 값에 영향을 주지 않고, 새로고침·하위 화면 왕복에는 그대로 남는다 — 이 화면이
 * 요구하는 "현재 설정 방문에만 유효" 범위와 정확히 겹친다. (기어를 새 탭으로 여는 경우는
 * 브라우저가 opener 의 sessionStorage 를 복제하므로 그 탭도 같은 source tab 으로 돌아간다.)
 *
